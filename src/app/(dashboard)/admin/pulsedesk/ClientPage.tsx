...
export default function ClientPage({ initialComplaints }: { initialComplaints: any[] }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (rawStr === lastRawRef.current) return // No changes, prevent re-renders
        lastRawRef.current = rawStr
  ...
  return (
    <div className="flex-1 p-6 pb-8 font-sans text-slate-300 flex flex-col h-[100vh]" style={{ backgroundColor: 'var(--color-bg-app)' }}>
      {/* KPI Banner */}
